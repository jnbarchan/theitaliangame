...
void BaizeScene::layoutCardsAsGroup(const QList<const Card *> &cards, bool isBadSetGroup /*= false*/, bool isInitialCardGroup /*= false*/)
{
    if (cards.count() == 0)
        return;
    ...
    const CardPixmapItem *leftmostItem = items.first();
    for (const CardPixmapItem *item : items)
